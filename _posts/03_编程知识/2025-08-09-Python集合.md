...
5. **可作为队列型映射结构**
   ...
   * 因为能在头尾插入/删除键值对，所以它常用来实现：
     ...
     * [**FIFO 队列**（先进先出映射）](/posts/栈与队列/#2-队列)
...
`Counter` 对象可以像普通字典一样比较是否相等。只有当两个 Counter 中的**所有键及其对应的计数**都完全一致时，`==` 才返回 `True`。
```python
from collections import Counter
...
从 **Python 3.10** 开始，`Counter` 支持富比较运算符（`<`, `<=`, `>`, `>=`），将其视为**多重集（Multiset）**的包含关系。

* `c1 <= c2`：表示 `c1` 是 `c2` 的子集。即对于 `c1` 中的每个键，其计数都小于或等于 `c2` 中对应键的计数。
* `c1 < c2`：表示 `c1` 是 `c2` 的真子集。
