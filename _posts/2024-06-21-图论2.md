...
|          | 0-1 BFS                 | 图中边权为 0/1 时的最短路径算法              | 熟悉   |
|          | 双端 BFS / 双向 BFS         | 加速两点之间路径搜索（如单词接龙、双向打通）          | 了解   |
|          | 欧拉路径 / 欧拉回路             | 一笔画问题，判定是否能不重复走遍所有边             | 了解   |
...
| **染色类**  | 二分图判定（DFS 染色）           | 判断是否为二分图，常用于图染色、匹配建模、矛盾类建模等     | 熟悉   |
| **匹配类**  | 匈牙利算法（Hungarian）        | 二分图最大匹配，常用于任务分配、稳定婚姻、匹配建模等      | 熟悉   |
| **搜索类**  | 回溯法                     | 图中路径、Hamilton 回路、组合图搜索类问题       | 了解   |
...
class UnionFind:
    def __init__(self, n):
        # 初始化：每个节点的父节点指向自己，秩初始化为 1
        self.parent = list(range(n + 1))  # 父节点数组
        self.rank = [1] * (n + 1)  # 秩数组，用于按秩合并
