...
单源最短路径问题在图论中极其常见，目标是在图中找到起点到各点之间路径总权重最小的路径。
...
**原理**：
采用**贪心策略**，每次选取当前距离最小的节点“锁定”其最短路径，然后松弛其邻居。使用**小根堆**优化效率。
...
**原理**：
Bellman-Ford 算法基于“松弛（Relaxation）”操作。
在图中（没有负环的情况下），一个最短路径最多只包含 $V - 1$ 条边（V 为顶点个数），因此只需对所有边进行 $V-1$ 次“松弛”操作，即可得到所有最短路径。
