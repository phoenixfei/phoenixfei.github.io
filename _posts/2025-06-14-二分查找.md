...
def binary_search_left(nums, target):
    ...
    ans = -1  # 默认返回-1，表示没找到更小的

    while left <= right:
        mid = (left + right) // 2
        if nums[mid] == target:
            ans = mid      # 只更新等于 target 的
            right = mid - 1
        elif nums[mid] < target:
            left = mid + 1
        else:
...
**对于 `ans` 的更新处理，也可以先宽松处理，最后再判断与 `target` 的关系。**
```python
if nums[mid] >= target:
    ans = mid
    right = mid - 1
...
> 该写法简单：不用在循环里加 `if nums[mid] == target` 分支，逻辑统一。
> 
> - 如果目标存在，这种写法是能找对的；
> - 如果目标不存在，后处理能检测出并返回 -1；
> 
> 同时，该写法支持「没找到时返回插入位置」，但 `ans` 应初始化为 `len(nums)`，或者改写代码为半开区间版本更简洁。
...
### 写法一：闭区间 \[left, right]
```python
def right_bound_last_occurrence(nums, target):
    left, right = 0, len(nums) - 1
    ans = -1
...
**同寻找左侧边界，对于 `ans` 的更新处理，也可以先宽松处理，最后再判断与 `target` 的关系。**
```python
def binary_search_left(nums, target):
    left, right = 0, len(nums) - 1
    ans = -1  # 表示初始没有找到答案
