...
这些问题可用「双关键字 Dijkstra」解决。

---

**核心思想**

> 将每个节点的状态定义为一个“二元组” `(primary, secondary)`，
...
* `dist[u] = (primary, secondary)`
  表示从起点到 u 的最佳状态。

2️⃣ 小根堆存储

* 每次从堆中弹出 `(primary, secondary, node)`
  其中比较规则为字典序。
...
* 对于每条边 `u → v`，尝试更新：
  ...
  ```python
  new = (primary + 1, secondary + w)
  if v not in dist or new < dist[v]:
      dist[v] = new
      heapq.heappush(pq, (new[0], new[1], v))
  ```
...
def dijkstra_two_keys(graph, start):
    """
    双关键字 Dijkstra
    graph: {u: [(v, w), ...]}
    start: 起点
    优先层级少的，再看路径距离
    """
    dist = {start: (0, 0)}  # node -> (层级, 距离)
    pq = [(0, 0, start)]    # (层级, 距离, 节点)

    while pq:
        level, d, u = heapq.heappop(pq)

        # 不写 if (level, d) > dist[u]: continue
        # 因为状态不唯一，不能简单判断“过期”

        for v, w in graph[u]:
            new = (level + 1, d + w)
            if v not in dist or new < dist[v]:  # 字典序比较
                dist[v] = new
                heapq.heappush(pq, (new[0], new[1], v))
...
不可以直接写`if (level, d) > dist[u]: continue`，因为双关键字 Dijkstra对于：

* 每个节点的状态是 `(level, d)`
* 可能存在多个路径到达同一节点 u：
  * 层级少、距离大
  * 层级多、距离小
* 这两种状态都可能在后续扩展中生成更优路径

如果写`if (level, d) > dist[u]: continue`，那么
* 弹出的某个状态可能被直接丢弃
* 导致后续无法利用这个状态生成最优路径
* **最终结果可能不是正确的最优路径**
...
| 特征    | 普通 Dijkstra                     | 双关键字 Dijkstra |
| ----- | ------------------------------- | ------------- |
| 状态    | 单值 `dist[u]`                    | 二元组 `(a,b)`   |
| 比较    | 直接数值比较                          | 字典序比较         |
| 状态唯一性 | ✅ 唯一                            | ❌ 可能多状态       |
| 过期判断  | ✅ 必须 `if d > dist[u]: continue` | 🚫 不能简单判断     |
| 应用场景  | 单目标最优路径                         | 多目标优先级路径      |

**可拓展到多关键字（多维 Dijkstra）**

如果你需要第三关键字（比如时间、风险等），
可扩展为三元组 `(key1, key2, key3)`，依然按字典序比较。
例如：
...
```python
if new < dist[v]:  # Python tuple自动按字典序比较
```
