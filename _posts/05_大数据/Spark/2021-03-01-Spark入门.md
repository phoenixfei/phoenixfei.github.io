...
### 3.1. 算子分类：Transformation 与 Action

Spark 中的算子，大体分为两类：**Transformation（转换类）** 与 **Action（动作类）**。

**Transformation：转换类算子（懒执行）**

Transformation 只负责描述“数据如何被一步步加工”，不会立即触发集群计算。

常见 Transformation 及作用：

* `map`：对每条数据做一对一映射，生成等量的新数据。
* `flatMap`：对每条数据映射出 0~N 条结果，并自动“拍平”。
* `filter`：按条件筛选，只保留满足条件的数据。
* `distinct`：按值去重，得到唯一元素集合（通常会触发 Shuffle）。
* `reduceByKey`：对相同 key 的 value 做聚合（如求和、计数），先本地聚合再全局聚合。
* `join`：按 key 把两个 RDD 关联起来，生成配对结果（通常会触发 Shuffle）。
...
* `count()`：返回数据总条数。
* `collect()`：把所有结果拉回 Driver 端（数据大时容易导致 Driver 内存压力）。
* `take()`：返回前 N 条数据，常用于快速查看样本。
* `saveAsTextFile()`：把结果写入外部存储（如 HDFS）。
* `foreach()`：对每条记录执行副作用操作（如写外部系统、打印日志）。
